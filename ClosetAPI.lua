...
function APIdebug:prepLog(debugvar,lmon,levelt,levelc,halt)
    ...
    if debugvar == false then 
        self.log = function(text,level)
            if level == halt-1 then self.panics[#self.panics+1] = text os.queueEvent("panic")  elseif
               level >= halt then self.panics[#self.panics+1] = text os.queueEvent("panic","halt") os.queueEvent("terminate") end
        end
        return
    end
